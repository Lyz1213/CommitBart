...
class Patch_Preprocessor:
    ...
    def input2Features(self, sample):
        input_ids, tgt_ids, type_ids = self.gen_token(sample)
        if input_ids == None:
            return None, None, None
        assert(input_ids.size(0) == self.max_len)
        assert (tgt_ids.size(0) == self.target_max_len)
        assert (type_ids.size() == input_ids.size())
        return input_ids, tgt_ids, type_ids


    def gen_token(self, sample):
        ...
        input_ids = m_ids + f_ids + d_ids
        inp_type_ids = [0] + m_type + f_type + d_type
        #label = 'True' if sample['vulnerable'] == 1 else 'False'
        label = 'security patch True' if sample['vulnerable'] == 1 else 'security patch False'
        tgt_ids = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(label))
        ...
        assert(len(tgt_ids) <= self.target_max_len)
...
def main():
    ...
    print(instances[16])
    for i in range(100):
        pre = Data_Preprocessor(0.2, 512, tokenizer)
