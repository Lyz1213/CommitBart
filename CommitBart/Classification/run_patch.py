...
from data_preprocess.f_patch_data import Patch_Preprocessor
    ...
    def __init__(self, preprocessor, args, isevaluate, lang, file_path, block_size=512):
        ...
        if os.path.exists(cached_features_file):
            ...
            logger.info("Creating features from dataset file at %s", os.path.join(file_path, lang + '_' + prefix +
                                                                                  '.jsonl.gz'))
            #data = self.load_jsonl_gz(os.path.join(file_path, lang + '_' + prefix + '.jsonl.gz'), self.tokenizer, args)
            data = self.load_jsonl_gz(os.path.join(file_path, lang + '_commits_' + prefix + '.jsonl.gz'))
            print('rank ' + str(args.local_rank)  + ' start preprocessing')
...
def train(args, train_datasets, model, preprocessor):
    ...
    for _, batch in merge_dataset(train_dataloaders, [len(x) for x in train_datasets], 0.7):
        ...
        loss, _, _ = model(gen_input_ids, gen_tgt_ids, gen_type_ids, input_embeds = True)
        if args.n_gpu > 1:
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0 and global_step >= 2000:
                ...
                output_dir = os.path.join(args.output_dir, '{}-{}-{}-{}-{}-{}'.format(checkpoint_prefix, global_step,
                                                                             round(results['acc'], 4), round(results['f1'],4),
                                                                                      round(results['pre'],4), round(results['rec'],4)))
...
def evaluate(args, model, preprocessor, eval_when_training=False):
    ...
    for eval_dataloader in eval_dataloaders:
        for gen_input_ids, gen_tgt_ids, gen_type_ids in iter(
            ...
            with torch.no_grad():
                _, loss, num= model(gen_input_ids, gen_tgt_ids, gen_type_ids, input_embeds = True)
...
def test(args, model, preprocessor):
    ...
    for eval_dataloader in eval_dataloaders:
        for source_ids, labels, type_ids in iter(eval_dataloader):
            ...
            #print('source _ids', preprocessor.tokenizer.convert_ids_to_tokens(source_ids[1]))

            labels = labels.to(args.device)
            #print('labels_ids', preprocessor.tokenizer.convert_ids_to_tokens(labels[1]))
            type_ids = type_ids.to(args.device) if args.type_ids else None
            #print('type ids', type_ids[1])
            for label in labels:
            ...
            with torch.no_grad():
                preds = model(gen_input_ids=source_ids, gen_type_ids=type_ids, input_embeds = True)
                for pred in preds:
                    t = pred[0].cpu().numpy()
                    t = list(t)
                    if 0 in t:
    ...
    fp = 0
    fn = 0
    for i in range(len(tgtss)):
        tgtss[i] = tgtss[i].strip().split()[-1]
        p[i] = p[i].strip().split()[-1]
        if tgtss[i] == 'True':
            if p[i] == 'True':
                tp += 1
            else:
            ...
            print('tgtss is ', tgtss[i])
    pre = float(tp) / float((tp + fp))
    rec = float(tp) / float((tp + fn))
    f1_score = float(2 * pre * rec) / float((pre + rec))
    acc = float(tp + tn) /  float(tp + tn + fp + fn)
    print('output ', args.output_dir)
    logger.info(" %s = %s " % ("acc", str(acc)))
    logger.info("  %s = %s " % ("f1", str(f1_score)))
    logger.info("  %s = %s " % ("pre", str(pre)))
    logger.info("  %s = %s " % ("rec", str(rec)))
    logger.info("  " + "*" * 20)
    result = {
        'acc': acc,
        'f1' : f1_score,
...
def main():
    ...
    target_size = 5
    data_pre = Patch_Preprocessor(args.block_size, target_size, tokenizer, args = args)
